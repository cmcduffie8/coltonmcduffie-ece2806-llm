...
def clean_data(text, valid_chars):
...
def get_batch(split,train_data,val_data,opt):
    ...
    if len(data) <= opt.block_size:
        raise ValueError(f"Data size {len(data)} is smaller than block size {opt.block_size}.")
